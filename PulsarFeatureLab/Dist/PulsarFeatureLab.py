...
class PulsarFeatureLab:
    ...
    def main(self,argv=None):
        ...
        # Helper files.
        utils = Utilities(self.verbose)
        dp    =  DataProcessor(self.verbose)
        
        # Process -d argument if provided. 
        if(utils.dirExists(self.dir)):
            self.searchLocalDirectory = False
        else:
            print("You must supply a valid input directory with -d flag.")
            sys.exit()
        ...
        if(utils.fileExists(self.out)):
            ...
            if(not utils.fileExists(self.out)):
                print("You must supply a valid output file path with -f flag.")
                sys.exit()
                
        # ADDING A NEW FEATURE? - the feature_type variable must change to use your feature.    
        ...
        # Process -i argument if provided. 
        ...
        if(self.arff == True):
            ...
            # ADDING A NEW FEATURE? Here you must make a change so that the code below writes out
            # enough @attribute lines for your features.
